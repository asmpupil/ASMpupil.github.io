...
function showMessage(text, type = 'info', time = 3000) {
...
function performFreeze(selectedArray) {
    try {
        ...
        setTimeout(() => {
            ...
            showMessage(`正在${selectedArray.join(', ')}磁盘冻结并重启设备,耗时较长,如果未在20s内重启,请重试.`, 'info', 20 * 1000);
            setTimeout(() => {
...
function performUnfreeze(selectedArray) {
    try {
        ...
        showMessage('正在解冻磁盘并重启设备... 请等待约5s...', 'info', 5000);
        setTimeout(() => {
            freezeAPI.executeReboot();
        }, 5 * 1000);
